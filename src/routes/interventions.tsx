  ...
  head: () => ({
    meta: [
      { title: "Gestion des interventions — EcoKin Smart" },
      {
        name: "description",
        content:
          "Affectation des équipes, suivi GPS des camions et validation par photo avant/après.",
      },
    ],
  }),
  component: () => (
            ...
            {trucks.map((t) => (
              ...
              >
                <span className="font-semibold">
                  {t.id} <span className="text-xs text-muted-foreground">· {t.driver}</span>
                </span>
              ...
              <article key={it.id} className="rounded-2xl border border-border bg-background p-4">
                ...
                <div className="mt-3 flex flex-wrap gap-3 text-[11px] text-muted-foreground">
                  <span className="inline-flex items-center gap-1">
                    <Users className="size-3" /> {it.team}
                  </span>
                  {it.truckId && (
                    <span className="inline-flex items-center gap-1">
                      <MapPin className="size-3" /> {it.truckId}
                    </span>
                  )}
