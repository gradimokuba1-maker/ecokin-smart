  ...
  head: () => ({
    meta: [
      { title: "Cartographie intelligente des itinéraires — EcoKin Smart" },
      {
        name: "description",
        content:
          "Planification et optimisation IA des tournées de collecte : itinéraires, points de collecte, distances et coûts de carburant.",
      },
    ],
  }),
  component: () => (
...
function ItinerairesPage() {
  ...
  const stats = useMemo(() => {
    ...
    const optKm = vehicles.reduce(
      (s, v) => s + routeDistanceKm(optimizeRoute(v.route, v.route[0])),
      0,
    );
      ...
      <header className="border-b border-border bg-kin text-white">
        <div className="mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8">
          ...
          <h1 className="mt-2 font-display text-4xl font-bold tracking-tight">
            Cartographie intelligente des itinéraires
          </h1>
          <p className="mt-2 max-w-2xl text-sm text-white/70">
            Planifiez les tournées, ajoutez des arrêts et optimisez automatiquement les parcours
            grâce à l'IA pour réduire le temps de déplacement et le coût de carburant.
          </p>
        </div>
      </header>
          ...
          <div className="overflow-hidden rounded-3xl border border-border bg-card">
            <div className="border-b border-border px-5 py-3">
              ...
              <h2 className="font-display text-lg font-bold">
                Itinéraires planifiés &amp; tournées en cours
              </h2>
            </div>
                  ...
                  <li key={v.id}>
                    ...
                    >
                      ...
                      <span className="text-xs font-semibold">
                        {routeDistanceKm(v.route).toFixed(1)} km
                      </span>
                    </button>
                  </li>
                ))}
              ...
              <div className="rounded-3xl border border-border bg-card p-5">
                ...
                <ol className="mt-3 space-y-1 text-xs">
                  {active.route.map((p, i) => (
                    ...
                    >
                      <span className="grid size-5 place-items-center rounded-full bg-eco text-[10px] font-bold text-white">
                        {i + 1}
                      </span>
                      <span>
                  ...
                  <ul className="mt-2 max-h-40 space-y-1 overflow-auto text-xs">
                    {COLLECTION_POINTS.map((cp) => (
                      <li key={cp.id}>
                        ...
                        >
                          <span>
                            <MapPin className="mr-1 inline size-3 text-eco" />
                            {cp.name}
                          </span>
            ...
            <table className="w-full text-xs">
              ...
              <tbody>
                {vehicles.map((v) => {
                  ...
                  const avg = v.track.length
                    ? v.track.reduce((s, f) => s + f.speedKmh, 0) / v.track.length
                    : v.current.speedKmh;
                  return (
