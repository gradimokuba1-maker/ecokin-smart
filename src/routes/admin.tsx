...
function AdminPage() {
  ...
  const scopedUsers = useMemo(
    () =>
      db.users.filter(
        (user) =>
          !session.commune || user.role === "gouverneur" || user.commune === session.commune,
      ),
      ...
      <div className="border-b bg-card">
        <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
          ...
          <p className="mt-1 text-muted-foreground">
            Gestion et supervision de l'ensemble des modules, utilisateurs et paramètres de la
            plateforme EcoKin Smart.
          </p>
        </div>
      </div>
      ...
      <PieChart>
        ...
        >
          {data.map((entry) => (
            <Cell
              key={`cell-${entry.name}`}
              fill={STATUS_CHART_COLORS[entry.name as LiveStatus] ?? "#8884d8"}
            />
          ))}
        </Pie>
        <Tooltip />
...
function Overview({
  ...
  const reportsByStatus = useMemo(() => {
    ...
    return Object.entries(counts).map(([status, count]) => ({
      name: STATUS_META[status as LiveStatus]?.label ?? status,
      value: count,
    }));
  }, [reports]);
...
function HouseholdsTab() {
  ...
  const filteredHouseholds = useMemo(() => {
    return households.filter((h) => {
      ...
      if (
        filters.q &&
        !`${h.name} ${h.address} ${h.phone}`.toLowerCase().includes(filters.q.toLowerCase())
      )
        return false;
      return true;
    });
    ...
    <div className="space-y-6">
      ...
      <div className="rounded-2xl border border-border bg-card p-6">
        <h3 className="mb-4 font-display text-lg font-bold">
          Gestion des ménages ({filteredHouseholds.length} / {households.length})
        </h3>
      ...
      <AlertDialog open={!!deletingUser} onOpenChange={(open) => !open && setDeletingUser(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Êtes-vous sûr de vouloir supprimer l'utilisateur "{deletingUser?.name}" ? Cette action
              est irréversible.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
    ...
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>Gérer les informations d'un compte citoyen.</DialogDescription>
        </DialogHeader>
...
function ReportsTab() {
  ...
  const filteredReports = useMemo(() => {
    ...
    return scopedReports.filter((r) => {
      ...
      if (filters.q && !`${r.id} ${r.description}`.toLowerCase().includes(filters.q.toLowerCase()))
        return false;
      return true;
    });
  ...
  return (
    <div className="rounded-2xl border border-border bg-card p-6">
      <h3 className="mb-4 font-display text-lg font-bold">
        Modération des signalements ({filteredReports.length} / {reports.length})
      </h3>
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            ...
            <Input
              ...
              onChange={(e) =>
                setCurrentRates((prev) => ({
                  ...prev,
                  bin: { ...prev.bin, "120L": Number(e.target.value) },
                }))
              }
            />
          </div>
          <div className="space-y-2">
            ...
            <Input
              ...
              onChange={(e) =>
                setCurrentRates((prev) => ({
                  ...prev,
                  bin: { ...prev.bin, "240L": Number(e.target.value) },
                }))
              }
            />
          </div>
          <div className="space-y-2">
            ...
            <Input
              ...
              onChange={(e) =>
                setCurrentRates((prev) => ({
                  ...prev,
                  bin: { ...prev.bin, "660L": Number(e.target.value) },
                }))
              }
            />
          </div>
          <div className="space-y-2">
            ...
            <Input
              ...
              type="number"
              step="0.1"
              value={currentRates.pmeMultiplier}
              onChange={(e) =>
                setCurrentRates((prev) => ({ ...prev, pmeMultiplier: Number(e.target.value) }))
              }
            />
          </div>
        </div>
...
function SettingsTab() {
  const { settings, updateSection, resetSettings, exportSettings, importSettings } =
    useAdminSettings();
  ...
  return (
    <div className="space-y-6">
      ...
      {settingsTab === "profile" && (
        <ProfileSettings profile={settings.profile} onUpdate={(v) => updateSection("profile", v)} />
      )}
      ...
      {settingsTab === "waste" && (
        <WasteCollectionSettingsSection
          waste={settings.wasteCollection}
          onUpdate={(v) => updateSection("wasteCollection", v)}
        />
      )}
      {settingsTab === "gis" && (
        <GisSettingsSection gis={settings.gis} onUpdate={(v) => updateSection("gis", v)} />
      )}
      {settingsTab === "ai" && (
        <AiSettingsSection ai={settings.ai} onUpdate={(v) => updateSection("ai", v)} />
      )}
      {settingsTab === "backup" && (
      ...
      {settingsTab === "danger" && (
        <div className="space-y-6">
          ...
          <div className="rounded-2xl border border-red-500/30 bg-red-500/5 p-6">
            <h3 className="mb-1 font-display text-lg font-bold text-red-700">
            ...
            {!confirmingReset ? (
              ...
              <div className="mt-4 flex flex-wrap items-center gap-2">
                ...
                {resetDone && (
                  <span className="text-xs font-semibold text-eco">✓ Données remises à zéro…</span>
                )}
              </div>
            )}
          </div>
          ...
          <div className="rounded-2xl border border-orange-500/30 bg-orange-500/5 p-6">
            <h3 className="mb-1 font-display text-lg font-bold text-orange-700">
          ...
          <div className="rounded-2xl border border-border bg-card p-6">
            ...
            <div className="mt-4 space-y-2">
              ...
              {importSuccess && (
                <p className="text-xs font-semibold text-eco">
...
function ProfileSettings({
  ...
  onUpdate: (v: Partial<AdminProfile>) => void;
}) {
...
function PlatformSettings({
  ...
  onUpdate: (v: Partial<PlatformConfig>) => void;
}) {
    ...
    <div className="rounded-2xl border border-border bg-card p-6">
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        ...
        <div className="flex items-center gap-3 pt-6">
          ...
          />
          <Label htmlFor="maintenance-mode" className="font-semibold text-red-600">
            Mode maintenance
          </Label>
        </div>
...
function NotificationSettingsSection({
  ...
  onUpdate: (v: Partial<NotificationSettings>) => void;
}) {
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        ...
        <div className="space-y-2">
          ...
          <Select
            ...
            onValueChange={(v) => setForm({ ...form, smsProvider: v })}
          >
            <SelectTrigger>
              <SelectValue />
            </SelectTrigger>
...
function SecuritySettingsSection({
  ...
  onUpdate: (v: Partial<SecuritySettings>) => void;
}) {
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        ...
        <div className="space-y-2">
          ...
          <Input
            ...
            onChange={(e) => setForm({ ...form, autoLogoutMinutes: Number(e.target.value) })}
          />
        </div>
        <div className="space-y-2">
        ...
        <div className="space-y-2">
          ...
          <Input
            ...
            onChange={(e) => setForm({ ...form, auditLogRetention: Number(e.target.value) })}
          />
        </div>
      </div>
      ...
      {form.ipWhitelistEnabled && (
        <div className="mt-4 space-y-2">
          ...
          <div className="flex gap-2">
            ...
            />
            <Button onClick={addIp} variant="outline" size="sm">
              Ajouter
            </Button>
          </div>
          <div className="flex flex-wrap gap-2">
            {form.ipWhitelist.map((ip) => (
              ...
              >
                {ip}
                <button onClick={() => removeIp(ip)} className="text-red-500 hover:text-red-700">
                  &times;
                </button>
              </span>
            ))}
...
function WasteCollectionSettingsSection({
  waste,
  onUpdate,
}: {
  waste: WasteCollectionSettings;
  onUpdate: (v: Partial<WasteCollectionSettings>) => void;
}) {
  ...
  return (
    <div className="rounded-2xl border border-border bg-card p-6">
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        <div className="space-y-2">
          ...
          <Input
            ...
            onChange={(e) => setForm({ ...form, collectionHours: e.target.value })}
          />
        </div>
        <div className="space-y-2">
          ...
          <Input
            ...
            onChange={(e) => setForm({ ...form, pmeMultiplier: Number(e.target.value) })}
          />
        </div>
        <div className="space-y-2">
          ...
          <Input
            ...
            onChange={(e) => setForm({ ...form, latePaymentPenalty: Number(e.target.value) })}
          />
        </div>
        <div className="space-y-2">
        ...
        <div className="space-y-2">
          ...
          <Input
            ...
            onChange={(e) =>
              setForm({ ...form, minimumVolumeForEmergency: Number(e.target.value) })
            }
          />
        </div>
        <div className="flex items-center gap-3 pt-6">
          <Switch
            ...
            onCheckedChange={(v) => setForm({ ...form, emergencyCollectionEnabled: v })}
          />
          <Label htmlFor="emergency-enabled" className="font-semibold">
            Collecte d'urgence activée
          </Label>
        </div>
      </div>
      ...
      <div className="mt-6">
        <h4 className="mb-3 text-sm font-bold uppercase tracking-widest text-muted-foreground">
          Fréquence de collecte par commune
        </h4>
        <p className="mb-2 text-xs text-muted-foreground">
          Définissez le nombre de collectes par semaine pour chaque commune.
        </p>
...
function GisSettingsSection({
  ...
  gis: GisSettings;
  onUpdate: (v: Partial<GisSettings>) => void;
}) {
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        <div className="space-y-2">
          ...
          <Select
            ...
            onValueChange={(v) => setForm({ ...form, mapProvider: v as any })}
          >
            <SelectTrigger>
              <SelectValue />
            </SelectTrigger>
        ...
        <div className="space-y-2">
          ...
          <Input
            ...
            onChange={(e) => setForm({ ...form, defaultZoom: Number(e.target.value) })}
          />
        </div>
        <div className="space-y-2">
...
function AiSettingsSection({
  ...
  ai: AiSettings;
  onUpdate: (v: Partial<AiSettings>) => void;
}) {
  ...
  return (
    <div className="rounded-2xl border border-border bg-card p-6">
      ...
      <div className="mb-6 flex items-center gap-3 rounded-xl border border-eco/30 bg-eco/5 p-4">
        ...
        />
        <div>
          <Label htmlFor="ai-enabled" className="font-bold text-eco">
            IA activée
          </Label>
          <p className="text-xs text-muted-foreground">
            Désactiver pour utiliser le mode dégradé (fallback)
          </p>
        </div>
      </div>
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        ...
        <div className="space-y-2">
          ...
          <Input
            ...
            onChange={(e) => setForm({ ...form, maxDailyApiCalls: Number(e.target.value) })}
          />
        </div>
      </div>
...
function BackupSettingsSection({
  ...
  onUpdate: (v: Partial<BackupSettings>) => void;
}) {
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        <div className="space-y-2">
          ...
          <Select
            ...
            onValueChange={(v) => setForm({ ...form, backupFrequency: v as any })}
          >
            <SelectTrigger>
              <SelectValue />
            </SelectTrigger>
    ...
    <div className="space-y-4">
      <div className="grid gap-4 md:grid-cols-3">
        <div className="rounded-2xl border border-eco/30 bg-eco/5 p-5">
          <div className="text-xs font-bold uppercase tracking-widest text-eco">
            Précision IA (apprentissage continu)
          </div>
          <div className="mt-1 font-display text-4xl font-bold">{precisionPct}%</div>
          <div className="mt-1 text-xs text-muted-foreground">
            {store.validations} validations · {store.corrections.length} corrections
          </div>
        </div>
      ...
      <div className="rounded-2xl border border-border bg-card p-6">
        <h3 className="mb-1 font-display text-lg font-bold">
          Validation des signalements en attente ({pendingReports.length})
        </h3>
        <p className="mb-4 text-sm text-muted-foreground">
          Validez la classification automatique ou corrigez-la. Chaque correction améliore le
          modèle.
        </p>
                ...
                {pendingReports.map((r) => (
                  <tr key={r.id} className="border-b border-border/60">
                    ...
                    <td className="capitalize">
                      {COMMUNES.find((c) => c.id === r.commune)?.name || r.commune}
                    </td>
          ...
          <ul className="space-y-1.5 font-mono text-xs">
            {store.corrections.slice(0, 10).map((c, i) => (
              <li key={i} className="text-muted-foreground">
                [{new Date(c.at).toLocaleString()}] {c.reportId} ·{" "}
                <span className="text-red-500">{c.predicted}</span> →{" "}
                <span className="text-eco">{c.corrected}</span> ({c.by})
              </li>
            ))}
          </ul>
        </div>
