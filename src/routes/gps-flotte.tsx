  ...
  head: () => ({
    meta: [
      { title: "Suivi GPS temps réel de la flotte — EcoKin Smart" },
      {
        name: "description",
        content:
          "Position en direct, vitesse, statut, historique des trajets et alertes automatiques (déviation, arrêt prolongé, hors ligne).",
      },
    ],
  }),
  component: () => (
    <AccessGate
      required={["agent", "bourgmestre", "gouverneur", "admin"]}
      ...
      <header className="border-b border-border bg-kin text-white">
        <div className="mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8">
          ...
          <p className="mt-2 max-w-2xl text-sm text-white/70">
            Position live, vitesse, statut, historique des trajets, alertes automatiques (déviation,
            arrêt prolongé, hors ligne) et relecture des tournées passées.
          </p>
        </div>
      </header>
      ...
      <main className="mx-auto max-w-7xl space-y-6 px-4 py-10 sm:px-6 lg:px-8">
        ...
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div className="text-xs text-muted-foreground inline-flex items-center gap-2">
            <span className="relative flex size-2">
              <span className="absolute inline-flex h-full w-full animate-ping rounded-full bg-eco/60" />
              ...
              <div className="rounded-3xl border border-border bg-card p-5">
                ...
                <dl className="mt-3 grid grid-cols-2 gap-2 text-xs">
                  ...
                  <Info
                    ...
                    value={
                      active.startedAt
                        ? new Date(active.startedAt).toLocaleTimeString("fr-FR")
                        : "—"
                    }
                  />
                  <Info
                    ...
                    value={
                      active.lastFixAt
                        ? new Date(active.lastFixAt).toLocaleTimeString("fr-FR")
                        : "—"
                    }
                  />
                </dl>
                ...
                <details className="mt-3">
                  <summary className="cursor-pointer text-xs font-semibold text-eco">
                    Relecture — {active.track.length} points
                  </summary>
                  <ul className="mt-2 max-h-48 space-y-1 overflow-auto text-[11px]">
                    {active.track.slice(0, 40).map((f, i) => (
                      ...
                      >
                        ...
                        <span>
                          {Math.round(f.speedKmh)} km/h · {f.lat.toFixed(4)}, {f.lng.toFixed(4)}
                        </span>
                      </li>
                    ))}
                  </ul>
        ...
        <section className="rounded-3xl border border-border bg-card p-5">
          <h3 className="font-display text-lg font-bold inline-flex items-center gap-2">
            <Activity className="size-4 text-flood" /> Alertes automatiques
          </h3>
          {alerts.length === 0 ? (
            <p className="mt-3 text-sm text-muted-foreground">
              Aucune anomalie détectée. La flotte est nominale.
            </p>
          ) : (
            <ul className="mt-3 space-y-1.5 text-sm">
              {alerts.map((a) => (
                ...
                >
                  <span>
                    <AlertTriangle className="mr-2 inline size-3 text-red-500" />
                    {a.msg}
                  </span>
