    ...
    return (
        <Card>
            <CardHeader>
                ...
                <CardDescription>
                    Points de collecte réels à Kinshasa.
                </CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
                <div className="flex gap-4">
                    <Select onValueChange={(value) => { setSelectedCommune(value); setSelectedQuartier(null); }} value={selectedCommune || ""}>
                ...
                {filteredZones.length > 0 ? (
                    <MapContainer center={[filteredZones[0].position.lat, filteredZones[0].position.lon]} zoom={14} scrollWheelZoom={false} style={{ height: "100%", width: "100%" }}>
                        <TileLayer
                            attribution='&copy; <a href="https://www.openstreetmap.org/copyright">OpenStreetMap</a> contributors'
                            url="https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png"
                        />
                        {filteredZones.map((zone) => (
                            <Marker key={zone.id} icon={zoneIcon} position={[zone.position.lat, zone.position.lon]}>
                                <Popup>
