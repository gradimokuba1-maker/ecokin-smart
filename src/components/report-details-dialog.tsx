...
export function ReportDetailsDialog({
  ...
  return (
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="w-[min(96vw,1100px)] max-w-[1100px] border-0 bg-background p-0 shadow-2xl sm:rounded-2xl">
        <DialogHeader className="border-b border-border px-6 py-4 text-left">
          <DialogTitle className="text-xl font-bold">Détails du Signalement: {report.id}</DialogTitle>
          <DialogDescription>
            Informations complètes du signalement et synthèse IA, affichées dans un panneau responsive.
          </DialogDescription>
        </DialogHeader>

        <div className="max-h-[82vh] overflow-y-auto overflow-x-hidden px-6 py-5">
              ...
              {(report.photoUrl || report.photoBefore) && (
                <div className="overflow-hidden rounded-2xl border border-border bg-black/5">
                  <img
                    ...
                    className="max-h-[360px] w-full rounded-2xl object-contain sm:max-h-[420px]"
                  />
                </div>
              )}
