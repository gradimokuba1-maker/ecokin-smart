        ...
        <tbody>
          {visible.map((report) => (
            <tr key={report.id} className="border-b border-border/60">
              ...
              <td>
                <span className={`rounded-full px-2 py-0.5 text-[10px] font-bold ${URGENCY_META[report.urgency]?.bg} ${URGENCY_META[report.urgency]?.color}`}>
                  {URGENCY_META[report.urgency]?.label}
                </span>
              </td>
              <td>
                <span className={`rounded-full px-2 py-0.5 text-[10px] font-bold ${STATUS_META[report.status]?.color}`}>
