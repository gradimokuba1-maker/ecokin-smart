...
function read(): CollectionOperationsState {
  ...
  try {
    ...
    return raw
      ? ({ ...DEFAULT_STATE, ...JSON.parse(raw) } as CollectionOperationsState)
      : DEFAULT_STATE;
  } catch {
...
export function useCollectionOperations() {
  ...
  return {
    ...
    registerCollector(
      input: Omit<IndependentCollector, "id" | "createdAt" | "missionsAssigned" | "history">,
    ) {
      const current = read();
    ...
    createMission(
      input: Omit<CollectionMission, "id" | "createdAt" | "progressPercent" | "status" | "history">,
    ) {
      const current = read();
      ...
      const nextMissions = current.missions.map((mission) =>
        mission.id === id
          ? {
              ...
              history: [
                ...mission.history,
                { label: "Mission assignée", at: new Date().toISOString() },
              ],
            }
          : mission,
      );
