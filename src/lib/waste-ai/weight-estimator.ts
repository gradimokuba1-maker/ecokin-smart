...
export function estimateWasteWeight(
    ...
    for (const entry of usable.length ? usable : [{ material: "inconnu" as const, percentage: 100 }]) {
    ...
    const confidence = Math.max(0, Math.min(0.92, volumeConfidence * 0.65 + detectionConfidence * 0.35));
    ...
    const uncertaintyPercent = weightKg > 0
