...
export const defaultWasteAIAdapter: ModelAdapter = {
    detect: async (imageDataUrl, options) => {
        ...
        const hasServerKey = Boolean(process.env.LOVABLE_API_KEY);
    ...
    segment: async (imageDataUrl, detections, options) => {
        const runningOnServer = typeof window === "undefined";
        const hasServerKey = Boolean(process.env.LOVABLE_API_KEY);
        if (runningOnServer && hasServerKey) return serverWasteAIAdapter.segment(imageDataUrl, detections, options);
    ...
    estimateVolume: async (imageDataUrl, segments, options) => {
        const runningOnServer = typeof window === "undefined";
        const hasServerKey = Boolean(process.env.LOVABLE_API_KEY);
        if (runningOnServer && hasServerKey) return serverWasteAIAdapter.estimateVolume(imageDataUrl, segments, options);
