...
function normalizeMaterial(label: string): WasteMaterial {
  ...
  if (
    ...
    return "metal";
  if (s.includes("food") || s.includes("organic") || s.includes("vegetable") || s.includes("fruit"))
    return "organique";
  if (
    s.includes("electro") ||
    s.includes("phone") ||
...
function buildComment(category: WasteMaterial, secondaryCategories: WasteMaterial[]): string {
  ...
  const secondary =
    secondaryCategories[0] && secondaryCategories[0] !== "inconnu"
      ? ` avec des traces de ${secondaryCategories[0]}`
      : "";
  return `Analyse visuelle : dépôt principalement composé de ${primary}${secondary}.`;
}

function extractTopMaterials(objects: DetectionResult["objects"]): {
  dominant: WasteMaterial;
  ...
  const confidence =
    count > 0 ? Math.max(0.2, Math.min(0.98, totalConfidence / Math.max(1, count))) : 0;
...
export interface WasteDetectionService {
  analyzeImage(
    imageDataUrl: string,
    options?: { signal?: AbortSignal; minConfidence?: number },
  ): Promise<WasteDetectionAnalysis>;
}
...
class LovableVisionWasteDetectionService implements WasteDetectionService {
  async analyzeImage(
    ...
    try {
          ...
          messages: [
            {
              role: "system",
              content:
                "Tu es un service de vision dédié à la classification de déchets. Réponds uniquement en JSON avec les clés dominantCategory, secondaryCategories, confidence et comment.",
            },
            {
              role: "user",
      ...
      const dominant = normalizeMaterial(
        String(parsed?.dominantCategory || parsed?.mainCategory || parsed?.category || "inconnu"),
      );
  ...
  private async fallback(imageDataUrl: string): Promise<WasteDetectionAnalysis> {
    try {
      ...
      return {
        ...
        secondaryCategories: [],
        confidence: 0,
        comment: "Analyse visuelle indisponible : aucun résultat fiable n'a été produit.",
        provider: "rule-based",
      };
    }
  }
...
export function createWasteDetectionService(provider?: string): WasteDetectionService {
  const requested =
    provider ??
    process.env.WASTE_DETECTION_PROVIDER ??
    (process.env.LOVABLE_API_KEY ? "lovable-vision" : "rule-based");
